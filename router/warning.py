...
@router.get('/community/warnings/{community_patron}', status_code=status.HTTP_200_OK,
            dependencies=[Depends(verify_user_access_token)], summary="Warnings",
            description="Get all community warnings")
async def get_ten_community_warnings(community_patron: str, total: int = Query(default=10, ge=1),
...
@router.get('/community/warnings/{community_patron}/paginated/{page}/{page_size}', status_code=status.HTTP_200_OK,
            dependencies=[Depends(verify_user_access_token)], summary="Warnings",
            description="Get all community warnings by pagination")
async def get_all_community_warnings_by_pagination(community_patron: str, page: int = 1, page_size: int = 100,
                                                   user: dict = Depends(verify_user_access_token)):
    community = await community_crud.get_community_by_patron(community_patron)
...
@router.get('/community/warning/{warning_id}', status_code=status.HTTP_200_OK,
            dependencies=[Depends(verify_user_access_token)], summary="Warnings", description="Get warning by id")
...
@router.post('/community/warnings', status_code=status.HTTP_201_CREATED,
             dependencies=[Depends(verify_user_access_token)], summary="Warnings", description="Create a warning")
...
@router.put('/community/warnings/{warning_id}', status_code=status.HTTP_204_NO_CONTENT,
            dependencies=[Depends(verify_user_access_token)], summary="Warnings", description="Update a warning by id")
async def update_community_warning(warning: UpdateWarningModel, warning_id: str,
                                   user: dict = Depends(verify_user_access_token)):
    #if is_parish_leader(user['position']) or is_council_member(user['position']):
    warning = dict(warning)
...
@router.delete('/community/warnings/{warning_id}', status_code=status.HTTP_204_NO_CONTENT,
               dependencies=[Depends(verify_user_access_token)], summary="Warnings",
