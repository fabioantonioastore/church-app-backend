...
@router.post('/community', status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_user_access_token)],
             summary="Community", description="Create a community")
...
@router.get('/community/{community_patron}', status_code=status.HTTP_200_OK,
            dependencies=[Depends(verify_user_access_token)], summary="Community",
...
@router.put('/community/{community_patron}', status_code=status.HTTP_204_NO_CONTENT,
            dependencies=[Depends(verify_user_access_token)], summary="Community",
            description="Update community by patron")
async def update_community(community_patron: str, community_data: UpdateCommunityModel,
    ...
    await community_crud.update_community(update_community_data(community, community_data))
    return {"community updated"}
    #raise unauthorized("You can't update community")


@router.delete('/community/deactive/{community_patron}', status_code=status.HTTP_204_NO_CONTENT,
               dependencies=[Depends(verify_user_access_token)], summary="Community",
...
@router.get('/community/{community_patron}/users', status_code=status.HTTP_200_OK,
            dependencies=[Depends(verify_user_access_token)], summary="Community",
...
@router.patch('/community/active/{community_patron}', status_code=status.HTTP_204_NO_CONTENT,
              dependencies=[Depends(verify_user_access_token)], summary="Community", description="Activate community")
...
@router.get('/community/{community_patron}/councils', status_code=status.HTTP_200_OK,
