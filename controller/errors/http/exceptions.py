...
def precondition_required(message: str = None):
    raise HTTPException(
        status_code=status.HTTP_428_PRECONDITION_REQUIRED, detail=message
    )
...
def http_version_not_supported(message: str = None):
    raise HTTPException(
        status_code=status.HTTP_505_HTTP_VERSION_NOT_SUPPORTED, detail=message
    )
...
def network_authentication_required(message: str = None):
    raise HTTPException(
        status_code=status.HTTP_511_NETWORK_AUTHENTICATION_REQUIRED, detail=message
    )
